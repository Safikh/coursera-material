...
def load_data_from_gcs(data_gcs_path: str) -> pd.DataFrame:
    ...
    logging.info(f"reading gs data: {data_gcs_path}")
...
def load_data_from_bq(bq_uri: str) -> pd.DataFrame:
    ...
    logging.info(f"reading bq data: {bq_uri}")
...
Training Model Parameters: {json.dumps(model_params)}
...
{{
    "instances": {json.dumps(example_data.tolist())}
}}
"""
    return report
